...
function loadConfig() {
    ...
    if (!rpcUrl || !jwt || !gateway || !privateKey) {
        throw new Error("Missing required env vars: CHAIN_RPC_URL, PINATA_JWT, PINATA_GATEWAY, DELEGATOR_ETH_PRIVATE_KEY");
    }
    _config = {
        rpcUrl,
        jwt,
        ...
        contentRegistryAddress: process.env.DS_REGISTRY_ADDR,
...
function deriveVaultId(name) {
    return keccak256(encodeAbiParameters(parseAbiParameters("string, address"), [
        name,
        getAccount().address,
    ]));
}
async function getFangorn() {
    if (_fangorn)
        return _fangorn;
    ...
    _fangorn = await Fangorn.init(walletClient, storage, litClient, domain, appConfig);
    return _fangorn;
}
// CLI setup
...
program
    ...
    .action(async (name, files, options) => {
    try {
...
program
    ...
    try {
        ...
        if (!manifest) {
            ...
            console.log("Upload data with `fangorn upload <vaultName> <file> --price <set-price>");
            process.exit(0);
        }
        console.log(`Vault: ${name} (${vaultId})`);
        console.log(`Entries (${manifest.entries.length}):`);
        for (const entry of manifest.entries) {
            console.log(`  - ${entry.tag} | price: ${entry.price} | cid: ${entry.cid}`);
        }
        process.exit(0);
    }
    catch (err) {
