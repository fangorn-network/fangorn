...
export class TestBed {
    ...
    static async init(delegatorWalletClient, delegateeWalletClient, jwt, gateway, litActionCid, 
    // circuitJsonCid: string,
    contentRegistryContractAddress, usdcContractAddress, rpcUrl) {
        // if (!circuitJsonCid) {
        ...
        const fangorn = await Fangorn.init(delegatorWalletClient, delegatorStorage, litClient, domain, config);
        const delegateeFangorn = await Fangorn.init(delegateeWalletClient, delegateeStorage, delegateeLitClient, domain, config);
    ...
    async payForFile(vaultId, tag, amount, to) {
        const auth = await this.buildUsdcAuthorization(to, amount, 84532, this.config.usdcContractAddress);
