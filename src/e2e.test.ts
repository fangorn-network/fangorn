...
describe("Fangorn basic encryption works", () => {
    ...
    beforeAll(async () => {
        ...
        if (!schemaRegistryAddress) throw new Error("SCHEMA_REGISTRY_ADDR required");

        dataSourceRegistryAddress = process.env.DS_REGISTRY_ADDR! as Address;
        if (!dataSourceRegistryAddress) throw new Error("DS_REGISTRY_ADDR required");

        settlementTrackerAddress = process.env.SETTLEMENT_TRACKER_ADDR! as Address;
        if (!settlementTrackerAddress) throw new Error("SETTLEMENT_TRACKER_ADDR required")
