...
export async function resolveViewSources(
    ...
    const resolveOwnedRid = async (spec: string, kind: string): Promise<Hex> => {
        ...
        const schemaId = registry.schemaId(name);
        if (!(await registry.getBucketSchemaExists(owner, schemaId))) throw new Error(`${kind} "${name}" is not registered — publish it first.`);
        sourceSchemas.add(schemaId);
        const ds = datasetLabel ?? `${schemaId}:${owner}`;
        const rid = resourceId(owner, schemaId, ds);
