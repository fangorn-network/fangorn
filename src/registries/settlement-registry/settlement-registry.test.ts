...
const MOCK_PRICE = 1_000_000n; // 1 USDC
...
// generateProof is async and pulls in heavy WASM/worker deps; replace entirely.
vi.mock("@semaphore-protocol/proof", () => ({
    generateProof: vi.fn(async () => MOCK_PROOF),
}));

// addMember must be on the prototype (not a class field) so vi.spyOn can find it.
...
// privateKeyToAccount does real elliptic-curve work; stub it out.
vi.mock("viem/accounts", () => ({
    privateKeyToAccount: vi.fn((key: Hex) => ({ address: OWNER_ADDRESS, key })),
}));

// parseSignature validates the v byte (must be 27 or 28); stub the output so
// signTypedData mocks don't need to produce a byte-perfect 65-byte signature.
// createWalletClient spins up a real HTTP transport; replace with a spy.
...
function makeClients({
    ...
    const walletClient = {
        ...
        // signTypedData return value doesn't matter — parseSignature is mocked above.
...
describe("SettlementRegistry", () => {
    ...
    describe("deriveResourceId (static)", () => {
        it("returns a 32-byte hex string", () => {
            const id = SettlementRegistry.deriveResourceId(OWNER_ADDRESS, "music.track.v1", "track-001");
            expect(id).toMatch(/^0x[0-9a-fA-F]{64}$/);
        });

        it("is deterministic — same inputs produce the same id", () => {
            const a = SettlementRegistry.deriveResourceId(OWNER_ADDRESS, "music.track.v1", "track-001");
            const b = SettlementRegistry.deriveResourceId(OWNER_ADDRESS, "music.track.v1", "track-001");
            expect(a).toBe(b);
        });

        it("produces different ids for different tags", () => {
            const a = SettlementRegistry.deriveResourceId(OWNER_ADDRESS, "music.track.v1", "track-001");
            const b = SettlementRegistry.deriveResourceId(OWNER_ADDRESS, "music.track.v1", "track-002");
            expect(a).not.toBe(b);
        });

        it("produces different ids for different schema names", () => {
            const a = SettlementRegistry.deriveResourceId(OWNER_ADDRESS, "schema.a", "tag");
            const b = SettlementRegistry.deriveResourceId(OWNER_ADDRESS, "schema.b", "tag");
            expect(a).not.toBe(b);
        });

        it("produces different ids for different owners", () => {
            const other: Address = "0x9999999999999999999999999999999999999999";
            const a = SettlementRegistry.deriveResourceId(OWNER_ADDRESS, "schema", "tag");
            const b = SettlementRegistry.deriveResourceId(other, "schema", "tag");
            expect(a).not.toBe(b);
        });
    });
    ...
    describe("createResource", () => {
        ...
        it("throws if the wallet has no account", async () => {
            ...
            await expect(registry.createResource(MOCK_RESOURCE_ID, MOCK_PRICE)).rejects.toThrow(
    ...
    describe("prepareTransferWithAuth", () => {
        ...
        it("returns a payload with v, r, s from parseSignature", async () => {
            const { registry } = makeRegistry();
            const payload = await registry.prepareTransferWithAuth(baseParams);
            // Values come from the parseSignature mock defined above.
        ...
        it("throws if walletClient has no chain", async () => {
            ...
            await expect(registry.prepareTransferWithAuth(baseParams)).rejects.toThrow(
                "must have a chain"
            );
        });
        ...
        it("uses a provided walletClient override instead of the instance client", async () => {
            const { registry } = makeRegistry();
            ...
            await registry.prepareTransferWithAuth({
                ...baseParams,
                walletClient: overrideWallet as never,
            });
    ...
    describe("prepareSettle", () => {
        ...
        it("returns a PrepareSettleResult with bigint proof fields", async () => {
            ...
            const result = await registry.prepareSettle({
                resourceId: MOCK_RESOURCE_ID,
                identity: {} as never,
                stealthAddress: STEALTH_ADDRESS,
            });
    ...
    describe("settle", () => {
        ...
        it("throws if walletClient has no chain", async () => {
            ...
            await expect(
                registry.settle({ relayerPrivateKey: MOCK_RELAYER_KEY, preparedSettle })
            ).rejects.toThrow("must have a chain");
        });
    });
    ...
    describe("isSettled", () => {
        ...
        it("returns false when the contract denies settlement", async () => {
        ...
        it("calls readContract with the correct args", async () => {
            ...
            expect(publicClient.readContract).toHaveBeenCalledWith(
                expect.objectContaining({
                    ...
                    args: [STEALTH_ADDRESS, MOCK_RESOURCE_ID],
                })
            );
        });
    });
        ...
        it("calls readContract with the correct args", async () => {
            ...
            expect(publicClient.readContract).toHaveBeenCalledWith(
                expect.objectContaining({
                    ...
                    args: [MOCK_RESOURCE_ID, MOCK_IDENTITY_COMMITMENT],
                })
            );
        });
    });
    ...
    // -------------------------------------------------------------------------
    describe("getGroupId", () => {
        it("returns the group id from the contract", async () => {
    ...
    // -------------------------------------------------------------------------
    describe("getPrice", () => {
        it("returns the price from the contract", async () => {
